...
1. **レイアウト保存 (Save Layout)**:
   - 現在表示されているすべての有効なアプリケーションウィンドウの座標、サイズ、表示状態（通常・最大化・最小化）、Z順（Z-Order）をキャプチャし、JSON 形式でディスクに保存します。
...
3. **履歴管理 (Layout History)**:
   - 最大12個のスナップショット履歴をシステムトレイから選択して復元できます。
   - 履歴メニュー項目には、保存時にアクティブだった上位3つのプロセス名が併記され、ホバーするとそのスナップショットに含まれる全ウィンドウタイトルが一覧表示されます。
...
スナップショットは以下の JSON スキーマに従い、実行環境の `snapshots/` ディレクトリに保存されます。
...
### 4.2 コア常駐制御
* **AppContext.cs**:
  - `ApplicationContext` を継承し、メインUIを表示せずにトレイアイコンと右クリックメニューを管理します。
...
* **WindowEnumerator.cs**:
  ...
  - Windows API `MonitorFromWindow` からウィンドウに対応するモニターの `DeviceName` を特定します。
...
* **RestoreEngine.cs**:
  ...
  - ウィンドウハンドル（HWND）が変更されている場合、PID、クラス名、タイトル名等を段階的にマッチングするフォールバック検索処理を内包します。
...
* **HotkeyTextBox.cs**:
  ...
  - フォーカス時に背景を薄い紫（`RGB(245, 243, 255)`）へ変更して「Press keys...」とキー待ち受け状態を示し、ショートカット入力成功（またはエスケープ解除）時に自動でフォーカスを親へ逃がすUXを実装しています。
...
sequenceDiagram
    ...
    U->>AC: 復元実行
    AC->>SM: RestoreLatestSnapshot()
    SM->>RE: RestoreLayout(snapshot)
    RE->>RE: Settings.Load() (設定ロード)
    
    loop 各復元対象ウィンドウ (Z順背面から前面)
        RE->>RE: FindWindowHandle() (再マッチング探索)
        alt ウィンドウが存在する
...
1. **オンメモリ履歴キャッシュによるI/O遮断**:
   - 右クリックでシステムトレイメニューを開く際、ファイルI/Oを一切発生させず、起動時に一括取得され、保存時に差分追加されるメモリ上のキャッシュから最大12件の履歴リストを瞬時にレンダリングします。これによりUIのプチフリーズが解消されています。
2. **PID-ProcessName キャッシュ**:
   - `WindowEnumerator` および `RestoreEngine` 内のウィンドウ探索ループで、同一のプロセスの名前を繰り返し引かないようキャッシュするため、多数 of ウィンドウが存在する場合も極めて低負荷で巡回を完了させます。
3. **キーボードフォーカス制御**:
   - `HotkeyTextBox` は、ただ文字を書き換えるだけのテキストボックスではなく、入力完了と同時に `Parent.Focus()` により即座にフォーカスを奪い、直感的かつ安全にキーボードフック入力状態から脱出できる仕組みを備えています。
4. **低メモリフットプリント**:
